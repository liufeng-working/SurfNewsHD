...
- (void)viewDidLoad
{
...
#ifdef ipad
    webView.frame = CGRectMake(0.0f, 0.0f, 1024.0f, 704.0f);
	self.navigationController.navigationBar.tintColor = [UIColor hexChangeFloat:@"000000"];
    ...
    CGFloat webY = self.StateBarHeight;
    CGFloat width = CGRectGetWidth(self.view.frame);
    CGFloat height = CGRectGetHeight(self.view.frame) - webY;
    webView.frame = CGRectMake(0.0f, webY, width, height);
...
- (void)viewDidAppear:(BOOL)animated
{
    ...
    if (clientType == ChinaMobielOAuth) {
        oauth2Client =
        [[OAuth2Client alloc] initWithClientKey:kChinaMobileWeiboAppKey
        ...
        NSDictionary *additionalParams = [NSDictionary dictionaryWithObjectsAndKeys:@"code", @"response_type",
                                          @"web2.0", @"display", @"true", @"forcelogin", nil];
    #else
        NSDictionary *additionalParams = [NSDictionary dictionaryWithObjectsAndKeys:@"code", @"response_type",
