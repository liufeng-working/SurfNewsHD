...
#define MG_MIN_VIEW_WIDTH				00.0	// minimum width a view is allowed to become as a result of changing the splitPosition.
...
- (UIViewController *)masterViewController
{
	if (_viewControllers && [_viewControllers count] > 0) {
		NSObject *object = [_viewControllers objectAtIndex:0];
...
- (UIViewController *)detailViewController
{
	if (_viewControllers && [_viewControllers count] > 1) {
		NSObject *object = [_viewControllers objectAtIndex:1];
		if ([object isKindOfClass:[UIViewController class]]) {
            UIViewController *controller = (UIViewController*)object;
			return [[controller retain] autorelease];
		}
...
- (void)setDividerStyle:(MGSplitViewDividerStyle)newStyle
{
	...
	// Layout all views.
	[self layoutSubviews];
    self.view.backgroundColor = [UIColor hexChangeFloat:@"535353"];
}
